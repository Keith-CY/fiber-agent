...
export class Channel {
  ...
  private _balance: Map<string, bigint> = new Map()
  ...
  public sync = async () => {
    ...
    const balance: Map<string, bigint> = new Map()
    if (listFromNode.length) {
      for (const ch of listFromNode) {
        const { funding_udt_type_script_hash } = await this._db.channels.upsert(ch)

        listFromDb = listFromDb.filter((i) => i.channel_id !== ch.channel_id)

        if (ch.state.state_name === ChannelStateName.ChannelReady) {
          const id = funding_udt_type_script_hash || 'CKB'
          const assetAmount = BigInt(ch.local_balance)
          // + BigInt(ch.received_tlc_balance)
          let b = balance.get(id) ?? BigInt(0)
          balance.set(id, assetAmount + b)
        }
